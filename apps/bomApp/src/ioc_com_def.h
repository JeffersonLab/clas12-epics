...
static const iocshFuncDef Init_commDef  = {"Init_BOM",  0, 0 };
static const iocshFuncDef genStart_commDef = {"Start_BOM_CRATE", sizeof(genStartArgs_ptr)/sizeof(genStartArgs_ptr[0]), genStartArgs_ptr };

// Initializing function
static void  runInit_BOM( const iocshArgBuf* args ) 
{
  cout << "Initializing BOM Application " << endl;
...
static void  runStart_BOM_CRATE( const iocshArgBuf* args ) 
{
...
class IocShellComReg { 
 public:
  IocShellComReg( ) { 
    ...
    iocshRegister( &Init_commDef , runInit_BOM ); 
    iocshRegister( &genStart_commDef, runStart_BOM_CRATE ); 
  }
};
