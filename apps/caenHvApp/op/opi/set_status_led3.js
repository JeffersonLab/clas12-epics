...
{	
	...
	try { vmon = PVUtil.getDouble(pvs[2]); }
	catch (ee) {}
	...
	if (istatus2==0 && istatus>=0)
	{
		...
		else
		{
            if (theStatus == 0) {
                if (vmon<400) {
		  widget.setPropertyValue("foreground_color","MEDM_COLOR_25");
		  widget.setPropertyValue("background_color","MEDM_COLOR_25");
                }
                else {
		  widget.setPropertyValue("foreground_color","On");
		  widget.setPropertyValue("background_color","On");
                }
            }
            else {
