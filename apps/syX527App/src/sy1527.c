...
static char A1515param[MAX_PARAM][MAX_CAEN_NAME] = {
                "V0Set","I0Set","V1Set","I1Set","RUp","RDWn","Trip","SVMax","OnOrder","OffOrder",
                "VMon","IMon","IMonReal","IMonDet","Status","Temp","Pw","ImRange","ZCDetect","ZCAdjust"
};
...
sy1527GetBoard(unsigned int id, unsigned int board)
{
  ...
  for(ipar=0; ipar<nXXXXXparam; ipar++)
  {
    ...
    if(PARAM_TYPE_NUMERIC == Measure[id].board[board].partypes[ipar])
  ...
  else
  {
    ...
    for(i=0; i<NrOfSl; i++, m+=strlen(m)+1, d+=strlen(d)+1)
    {
      ...
      else
      {
          ...
          for(j=0; j<Measure[id].board[i].nparams; j++)
          {
            ...
            else
            {
              ...
              Demand[id].board[i].partypes[j] = tipo;
                ...
                !strcmp(Measure[id].board[i].modelname,"A1833B"))
        {
          //printf("---> found board %s\n",Measure[id].board[i].modelname);
        ...
        else if(!strcmp(Measure[id].board[i].modelname,"A1515BTG") ||
                !strcmp(Measure[id].board[i].modelname,"A1515BTGHP"))
        {
          //printf("---> found board %s\n",Measure[id].board[i].modelname);
          Measure[id].board[i].nparams = nA1515param;
          Demand[id].board[i].nparams = nA1515param;
          for(j=0; j<Measure[id].board[i].nparams; j++)
          {
            strcpy(Measure[id].board[i].parnames[j],A1515param[j]);
            strcpy(Demand[id].board[i].parnames[j],A1515param[j]);
            strcpy(ParName,Measure[id].board[i].parnames[j]);
            ...
            else
            {
              ...
              || !strcmp(Measure[id].board[i].modelname,"A7030SN")) {
