...
static pthread_t cratewatchdogthread; /// cratewatchdog
...
int mainframes[MAX_HVPS]; // list of id-s (where id -1 means crate is not opened for error)
int mainframes_all[MAX_HVPS]; // list of all id-s tryed to connect
...
int mbp=MAX_BOARDPARTS;
  ...
  if(pthread_create(cratewatchdogthread,NULL,crate_level_watchdog_thread_function,0)
  {
   printf("ERROR: pthread_create cratewatchdogthread (0x%08lx,...) failure\n",cratewatchdogthread);
   return(CAENHV_SYSERR);
  }
  else
  {
   printf("INFO: pthread_create cratewatchdogthread (0x%08lx,...) done\n", cratewatchdogthread);
  }

  return(CAENHV_OK);
}
//===========================================================================
void *crate_level_watchdog_thread_function(void *arg){  /// cratewatchdog

 static int TIME_INTERVAL_CRATE_WATCHDOG=1;
 static int TIME_OUT_CRATE_WATCHDOG=180/TIME_INTERVAL_CRATE_WATCHDOG; /// in seconds
 static int mainframes_disconnect_count[MAX_HVPS];

 for(i=0;i<MAX_HVPS;i++){
  mainframes_disconnect_count[i]=0;
 }
 int id_save;


 while(1){
  sleep(TIME_INTERVAL_CRATE_WATCHDOG);

  for(i=0;i<nmainframes;i++){
   if(mainframes[i]==-1 || mainframes_disconnect[i]==1){(mainframes_disconnect_count[i])++;}
    if(mainframes_disconnect_count[i] > TIME_OUT_CRATE_WATCHDOG){
     mainframes_disconnect_count[i]=0;
     id_save=mainframes_all[i];
     sy1527Stop(id_save);
     //mainframes[i]=-1; as we moved arrays
     sy1527Start(id_save,Measure[id_save].IPADDR);
    }
  }
...
/// my:  sprintf(name,"sy1527_%03d",id);
 ...
 /// if(ret == CAENHV_OK) {  cratewatchdog comment
    if(strlen(ip_address)>=MAX_CAEN_NAME){printf("too long mainfraime IP: exits now\n");exit(1);} /// my:
    strcpy(Measure[id].IPADDR,ip_address); /// my:
    mainframes_all[nmainframes-1]=id; 

 /// }
 /// else cratewatchdog comment
  if(ret != CAENHV_OK) /// cratewatchdog (insted of else)
  {
...
sy1527Stop(unsigned id)
{
  ...
  else
  {
    ...
    /// cratewatchdog comment pthread_mutex_unlock(&global_mutex);
    /// cratewatchdog comment return(CAENHV_SYSERR);
     Measure[id].id = -1;    /// cratewatchdog
  }
  ...
  for(i=j; i<nmainframes; i++) {
   mainframes[i] = mainframes[i+1];
   mainframes_all[i] = mainframes_all[i+1]; /// cratewatchdog
   mainframes_disconnect[i] = mainframes_disconnect[i+1]; /// cratewatchdog
  }
  nmainframes --;
