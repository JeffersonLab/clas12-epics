...
def makeSector(sector,xoff,yoff,angle):
  ...
  for ii in range(0,68):
    ...
    transformPoint(ptU,-diameter*4.0,diameter*0.0,-120)
    transformPoint(ptV,-diameter*1.0,diameter*5,   0)
    transformPoint(ptW, diameter*4.0,diameter*0.0, 120)
    pts.append(ptU)
    if ii>=62: continue
    pts.extend([ptV,ptW])
  for ii in range(0,36):
    ...
    diameter=12
    side = NECAL * diameter / 2
    height = side * math.sqrt(3) / 2
    x0V,y0V = x0 + side/2 , y0 - height/2
    x0U,y0U = x0 + side/2 , y0 - height/2
    x0W,y0W = x0          , y0 + height/2
    xV = x0V - ii * diameter / 2 - diameter * 1
    if ii%2==0: yV = y0V + diameter * math.sqrt(3)/2
    else:       yV = y0V
    ptU={'x':xV,'y':yV,'d':diameter,'pv':prefix+'_UI_E%.2d'%(ii+1)}
    ptV={'x':xV,'y':yV,'d':diameter,'pv':prefix+'_VI_E%.2d'%(ii+1)}
    ptW={'x':xV,'y':yV,'d':diameter,'pv':prefix+'_WI_E%.2d'%(ii+1)}
    transformPoint(ptU, diameter*0.0,diameter*0.5,-120)
    transformPoint(ptV,-diameter*0.0,diameter*0.5,   0)
    transformPoint(ptW,-diameter*0.5,diameter*0.0, 120)
    pts.extend([ptU,ptV,ptW])
  for ii in range(0,36):
    prefix='B_DET_ECAL_HV_SEC'+str(sector)
    diameter=10
    side = NECAL * diameter / 2
    ...
    ptU={'x':xV,'y':yV,'d':diameter,'pv':prefix+'_UO_E%.2d'%(ii+1)}
    ptV={'x':xV,'y':yV,'d':diameter,'pv':prefix+'_VO_E%.2d'%(ii+1)}
    ptW={'x':xV,'y':yV,'d':diameter,'pv':prefix+'_WO_E%.2d'%(ii+1)}
    transformPoint(ptU,-diameter*1.0,diameter*0.0,-120)
    transformPoint(ptV,-diameter*0.0,diameter*2.0,   0)
    transformPoint(ptW, diameter*1.0,diameter*0.0, 120)
    pts.extend([ptU,ptV,ptW])
