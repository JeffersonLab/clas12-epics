...
def makeLayer(sector,xoff,yoff,angle,diameter,pio):
  ...
  x0,y0=10,-10
  if pio=='P':
    prefix='B_DET_FTOF_HV_SEC'+str(sector)+'_PANEL1B'
    nelements=NFTOF['1B']
  elif pio=='I':
    prefix='B_DET_FTOF_HV_SEC'+str(sector)+'_PANEL1A'
    nelements=NFTOF['1A']
  ...
  for ii in range(0,nelements):
    ...
    jj = nelements - ii - 1
    ptU={'x':xV,'y':yV,'d':diameter,'pv':prefix+'_R_E%.2d'%(jj+1)}
    ptW={'x':xV,'y':yV,'d':diameter,'pv':prefix+'_L_E%.2d'%(jj+1)}
