...
WIDGTAIL='''
    ...
    <scripts>
      <path pathString="../../apps/mpodLvApp/scripts/led_status.js" checkConnect="true" sfe="false" seoe="false">
        <pv trig="true">^^^PREFIX^^^_SEC^^^SECTOR^^^_^^^LEFTRIGHT^^^^^^CHANNEL^^^</pv>
      </path>
    </scripts>
...
def genSector(sector,xoff,yoff,angle):
  ...
  for channel in range(4):
    ...
    points = [ [-2*channel-off,    0-off],
               [-2*(channel+1)+off,0-off],
               [-2*(channel+1)+off,-slope*(3+channel)+off],
               [-2*channel-off,    -slope*(2+channel)+off-0.12] ]
    ...
    for leftright in ['R','L']:
      ...
      asdf=asdf.replace('^^^CHANNEL^^^','%d'%(channel+1))
...
print HEAD
genSector(1,0,0,0)
genSector(2,45,-80,-60)
genSector(3,140,-80,-120)
genSector(4,185,0,-180)
genSector(5,140,80,120)
genSector(6,45,80,60)
print TAIL
