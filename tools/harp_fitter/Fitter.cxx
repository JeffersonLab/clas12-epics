...
void Fitter::ParseDataFile(std::string fname) {
    ...
    while (getline(dataFile, curLine)) {
        ...
        if (vecLine.size() == 4) {
            if (vecLine.at(1).compare("IPM2C21A_AVG") == 0) {
            ...
            } else if (vecLine.at(1).compare("IPM2C24A_AVG") == 0) {
...
void Fitter::SubmitToLogbook() {
    ...
    system(Form("/site/ace/certified/apps/bin/logentry %s -t \" Scan of %s \" -a %s -b \"%s\" ", logbook_list.c_str(), harp_name.c_str(), dump_img_name.c_str(), log_text_filename.c_str()));

    // When Log entry is done, let's delete the temporary file containing the log comments, in order
